      ...
      .then((data) => {
        setHeadways(
          (data.routes || []).map((r) => ({
            ...
            median_headway: r.median_headway,
            p90_headway: r.p90_headway,
            p95_headway: r.p95_headway,
            headway_cv: r.headway_cv,
            expected_headway: r.expected_headway,
          }))
        );
      })
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <p className="mb-6 text-sm text-slate-500">
        Neste lab, <strong>headway</strong> = intervalo entre pings GPS consecutivos do mesmo
        veículo (proxy de cadência de telemetria), não o intervalo entre chegadas em uma
        parada. Use a comparação com o headway esperado da rota como indicador exploratório.
      </p>
        ...
        <div className="overflow-x-auto rounded-lg border border-slate-200">
          <table className="min-w-full divide-y divide-slate-200 bg-white">
            <caption className="sr-only">
              Comparativo de headway mediano, P90, P95 e CV por rota
            </caption>
      ...
      {headways.length > 0 && (
        <div className="mt-6">
          <HeadwayDistribution
            headways={headways
              .filter((h) => h.median_headway !== null)
              .map((h) => h.median_headway as number)}
          />
        </div>
      )}
